...
Current RDBLearn version: 1.1 (FastDFS 1.1)
...
Use the **release tags** for this skill bundle (not branches): RDBLearn **v1.1**, FastDFS **v1.1**.

```bash
git clone --depth 1 https://github.com/HKUSHXLab/rdblearn.git ~/<skill-dir>/skills/use-rdblearn/codes/rdblearn/
cd ~/<skill-dir>/skills/use-rdblearn/codes/rdblearn && git checkout v1.1

git clone --depth 1 https://github.com/HKUSHXLab/fastdfs.git ~/<skill-dir>/skills/use-rdblearn/codes/fastdfs/
cd ~/<skill-dir>/skills/use-rdblearn/codes/fastdfs && git checkout v1.1
```

(`git clone --branch NAME` also accepts tag names, but `v1.1` tags on GitHub are not branches—`git checkout` after clone is the clearest approach.)
